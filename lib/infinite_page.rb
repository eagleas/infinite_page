...
module InfinitePage
  ...
  def infinite_page(collection = nil, options = {})
    unless WillPaginate::ViewHelpers.total_pages_for_collection(collection) <= 1
      ...
      options[:renderer] = 'WillPaginate::ViewHelpers::LinkRenderer'
  ...
  class ScrollObserverRenderer < WillPaginate::ViewHelpers::LinkRenderer
    def to_html
...
if defined?(Rails) and defined?(ActionController)
  ActionView::Base.send :include, InfinitePage unless ActionView::Base.instance_methods.include? 'infinite_page'
end
